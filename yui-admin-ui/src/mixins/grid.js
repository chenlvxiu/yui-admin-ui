...
export default {
  ...
  methods: {
    ...
    // 获取数据列表
    listData () {
      let currtSearchStr = JSON.stringify({...this.dataForm, ...this.params})
      // console.log(currtSearchStr)
      if (currtSearchStr !== this.searchStr) {
        this.pageNo = 1
        this.searchStr = currtSearchStr
      }
      ...
      this.$http.get(qryUrl, {
        ...
        this.total = this.gridOptions.isPage ? res.page.total : 0
      }).catch(() => {
        this.dataListLoading = false
      })
    },
    ...
    delHandle (id) {
      ...
      this.$confirm(`确定删除?`, '提示', {
        ...
        this.$http.get(this.gridOptions.delUrl, {
          params: {ids: ids.join(',')}
        }).then(({data: res}) => {
          if (res && res.code === 0) {
            this.$message({
