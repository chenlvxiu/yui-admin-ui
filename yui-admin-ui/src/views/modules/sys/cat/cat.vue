<template>
  <div class="mod-cat">
    ...
    <el-table :data="dataList" border stripe v-loading="dataListLoading" :max-height="tableHeight" 
      ...
      <el-table-column prop="cd" label="编码" sortable="custom" header-align="center" align="center">
        <template slot-scope="scope">
          <span v-if="dataForm.pid !== null">{{ scope.row.cd }}</span>
          <el-button v-else type="text" @click="childHandle(scope.row)">{{ scope.row.cd }}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="nm" label="名称" sortable="custom" header-align="center" align="center"></el-table-column>
      <el-table-column prop="seq" label="显示排序" sortable="custom" header-align="center" align="center"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      childHandle (row) {
        ...
        var routeName = `${this.$route.name}Itm__${row.id}_${row.cd}_${row.nm}`
        var route = window.SITE_CONFIG['dynamicRoutes'].filter(item => item.name === routeName)[0]
        var params = { 'pid': row.id, 'pnm': row.nm, 'pcd': row.cd }
        if (route) {
        ...
        route = {
          ...
          meta: {
            ...
            title: `${this.$route.meta.title} - ${row.cd}`
          }
        }
