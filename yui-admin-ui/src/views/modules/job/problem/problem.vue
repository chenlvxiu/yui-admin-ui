  ...
  <div class="mod-problem">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="listData()">
      ...
      <el-form-item>
        ...
        <el-button v-if="$hasPerm('job:problem:add')" size="small" type="primary" @click="childHandle(null)">新增</el-button>
        <el-button v-if="$hasPerm('job:problem:expt')" size="small" @click="exptHandle()">导出</el-button>
        <el-button v-if="$hasPerm('job:problem:impt')" size="small" @click="imptHandle()">导入</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border v-loading="dataListLoading" :max-height="tableHeight" 
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="220">
        <template slot-scope="scope">
          <el-button v-if="$hasPerm('job:problem:info')" type="text" size="small" @click="infoHandle(scope.row.id)" title="预览">预览</el-button>
          <!--<el-button v-if="$hasPerm('job:problem:audit')" type="text" size="small" @click="infoHandle(scope.row.id)" title="详情">详情</el-button>-->
          <el-button v-if="$hasPerm('job:problem:upd') && scope.row.isOwn===1 && scope.row.statCd==='30070.170.130'" type="text" size="small" @click="childHandle(scope.row.id)" title="修改">修改</el-button>
          <el-button v-if="$hasPerm('job:problem:info') && scope.row.statCd==='30070.170.130'" type="text" size="small" @click="reasonHandle(scope.row.id)" title="拒绝原因">拒绝原因</el-button>
          <el-button v-if="$hasPerm('job:problem:del') && scope.row.isOwn===1" type="text" size="small" @click="delHandle(scope.row.id)" title="删除">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import edit from './problem-edit'
  import reason from './problem-reason'
  import impt from './problem-impt'
  ...
  export default {
    ...
    methods: {
      ...
      childHandle (id) {
        ...
        if(!id)
        {
            return this.$router.push({ name: 'problem-add', params: params })
        }
        else 
        {
             return this.$router.push({ name: 'problem-edit', params: params })
        }
      }
    },
    components: {
