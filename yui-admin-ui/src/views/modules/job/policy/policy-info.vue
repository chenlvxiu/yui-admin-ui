<template>
  <el-dialog title="预览" class="mod-policy-info details"
    ...
    <div class="details-inner" v-loading="loading">
      <div class="details-title flex-column">
        ...
        <div class="details-title-desc flex-row">
          ...
          <p>公布文件号：<span>{{details.fileCd}}</span></p>
        </div>
      </div>
      ...
      <div class="details-download" v-if="details.fileDtoList&&details.fileDtoList.length">
        ...
        <div class="flex-column">
          <a class="details-download-item flex-row" v-for="(item,key) in details.fileDtoList" :key="key"
             :href="item.fileUrl" :download="item.name">
      ...
      auditPass() {
        this.$confirm(`确认审核通过?`, '提示', {
          ...
          if(this.details.statCd==='30070.170.110') {
            this.$http.post('/job/policy/audit',{id:this.id,statCd:'30070.170.120'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
            ...
            this.$http.post('/job/policy/audit',{id:this.id,statCd:'30070.170.140'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
