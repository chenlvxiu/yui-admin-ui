  ...
  <div  class="mod-policy-edit">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="120px">
      <el-row :gutter="20" class="row_top">
        ...
        <el-col :span="6">
          <el-form-item label="公布文件号：" prop="fileCd">
            <el-input v-model.number="dataForm.fileCd" placeholder="请输入公布文件号"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
...
<script>
  ...
  export default {
    data () {
      ...
      return {
        ...
        dataRule: {
          ...
          fileCd: [
            { required: true, message: '公布文件号不能为空', trigger: 'blur' },
            { min: 1, max: 200, message: '公布文件号长度在 1 到 200 个字符', trigger: 'blur' }
          ],
          content: [
    ...
    activated () {
        ...
        if(!this.id && this.$route.name === 'policy-edit')//编辑页面第一次进来获取传过来的id，防止点击tab页异常
          this.id = this.$route.params.id ;
        this.$refs['dataForm'].resetFields()
        if(this.id && this.$route.name === 'policy-edit')//编辑页获取用户信息
        {
           this.getInfo()  
        }
    },
    methods: {
      ...
      view () {
       this.$refs['dataForm'].validate((valid) => {
          if (valid) {
	        ...
	        this.details.fileDtoList = this.dataForm.jobPolicyFileRoList
	        this.$nextTick(() => {
      ...
      // 获取职称申报系统-政策文件表信息
      getInfo () {
        this.$http.get('/job/policy/webInfo?id=' + this.id).then(({data: res}) => {
          if (res.code !== 0) {
          ...
          this.dataForm.jobPolicyFileRoList = this.dataForm.fileDtoList
        }).catch(() => {})
      },
       // 获取编辑器内容 格式为html
      getEditConts (value) {
      ...
      uploadHandle (val) {
        ...
        this.$set(this.dataForm, 'jobPolicyFileRoList', val)
      },
      // tabs, 删除tab
      ...
      dataFormSubmit: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            let url = !this.id ? '/job/policy/add' : '/job/policy/upd'
            /*if(this.id) {
