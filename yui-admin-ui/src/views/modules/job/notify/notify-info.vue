<template>
  <el-dialog title="预览" class="mod-notify-info details"
...
<script>
  import audit from './notify-audit'
  export default {
    data() {
      return {
        id: null,
    ...
    methods: {
      ...
      auditPass () {
      	this.$confirm(`确认审核通过?`, '提示', {
            ...
            this.$http.post('/job/notify/audit',{id:this.id,statCd:'30070.170.120'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
            ...
            this.$http.post('/job/notify/audit',{id:this.id,statCd:'30070.170.140'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
