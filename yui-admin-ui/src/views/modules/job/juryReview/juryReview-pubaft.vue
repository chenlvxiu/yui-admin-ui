<template>
  <el-dialog :title="'评后公示'"  class="mod-pubbef"
  ...
  export default {
    data () {
      var validateTimer = (rule, value, callback) => {
        ...
        if (!/\S/.test(value)) {
          callback(new Error('评后公示时间范围不能为空'))
        } else if(Date.parse(new Date()) >= Date.parse(value[1])){
          callback(new Error('评后公示结束时间必须大于今天日期!'))
        }
        else {
           callback()
        }
    ...
    methods: {
      ...
      getInfo () {
        this.$http.get('/job/juryReview/info/' + this.dataForm.id).then(({data: res}) => {
          ...
          this.dataForm.daterange = [this.dataForm.phStartTm, this.dataForm.phEndTm]
        }).catch(() => {})
      },
      // 表单提交
      dataFormSubmit: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.dataForm.phStartTm =  Date.parse(this.dataForm.daterange[0])
            this.dataForm.phEndTm =  Date.parse(this.dataForm.daterange[1])
            //开始日期小于等于当前日期
            if(Date.parse(new Date()) >= Date.parse(this.dataForm.daterange[0])) {
                 this.dataForm.statCd = '30070.130.170';
            }
