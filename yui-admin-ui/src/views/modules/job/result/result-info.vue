<template>
  <el-dialog title="预览" class="mod-result-info details"
...
<script>
  import audit from './result-audit'
  export default {
    data() {
      return {
        id: null,
    ...
    methods: {
      ...
      auditPass () {
          ...
          if(this.details.statCd==='30070.170.110'){
            this.$http.post('/job/result/audit',{id:this.id,statCd:'30070.170.120'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
            ...
            this.$http.post('/job/result/audit',{id:this.id,statCd:'30070.170.140'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
