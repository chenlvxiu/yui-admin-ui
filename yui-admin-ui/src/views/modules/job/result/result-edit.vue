...
<script>
  ...
  export default {
      ...
      return {
        ...
        dataRule: {
          ...
          views: [
            ...
            // { min: 0, max: 2, message: '浏览量不超过999999', trigger: 'blur' }
          ],
          content: [
    ...
    activated () {
        ...
        if(!this.id && this.$route.name === 'result-edit')//编辑页面第一次进来获取传过来的id，防止点击tab页异常
          this.id = this.$route.params.id ;
        this.$refs['dataForm'].resetFields()
        if(this.id && this.$route.name === 'result-edit')//编辑页获取用户信息
        {
           this.getInfo()  
        }
    },
    methods: {
      ...
      view () {
       this.$refs['dataForm'].validate((valid) => {
          if (valid) {
	        ...
	        this.details.fileDtoList = this.dataForm.jobResultFileRoList
	        this.$nextTick(() => {
      ...
      // 获取职称申报系统-政策文件表信息
      getInfo () {
        this.$http.get('/job/result/webInfo?id=' + this.id).then(({data: res}) => {
          if (res.code !== 0) {
          ...
          this.dataForm.jobResultFileRoList = this.dataForm.fileDtoList
        }).catch(() => {})
      },
       // 获取编辑器内容 格式为html
      getEditConts (value) {
      ...
      uploadHandle (val) {
        ...
        this.$set(this.dataForm, 'jobResultFileRoList', val)
      },
      // tabs, 删除tab
      ...
      dataFormSubmit: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            let url = !this.id ? '/job/result/add' : '/job/result/upd'
            /*if(this.id) {
