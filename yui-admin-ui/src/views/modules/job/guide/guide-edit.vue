...
<script>
  ...
  export default {
    ...
    activated () {
        ...
        if(!this.id && this.$route.name === 'guide-edit')//编辑页面第一次进来获取传过来的id，防止点击tab页异常
          this.id = this.$route.params.id ;
        this.$refs['dataForm'].resetFields()
        if(this.id && this.$route.name === 'guide-edit')//编辑页获取用户信息
        {
           this.getInfo()  
        }
    },
    methods: {
      ...
      view () {
       this.$refs['dataForm'].validate((valid) => {
          if (valid) {
	        ...
	        this.details.fileDtoList = this.dataForm.jobGuideFileRoList
	        this.$nextTick(() => {
      ...
      getInfo () {
        this.$http.get('/job/guide/webInfo?id=' + this.id).then(({data: res}) => {
          ...
          this.dataForm.jobGuideFileRoList = this.dataForm.fileDtoList
        }).catch(() => {})
      },
       // 获取编辑器内容 格式为html
      getEditConts (value) {
      ...
      uploadHandle (val) {
        ...
        this.$set(this.dataForm, 'jobguideFileRoList', val)
      },
      // tabs, 删除tab
      ...
      dataFormSubmit: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            let url = !this.id ? '/job/guide/add' : '/job/guide/upd'
            /*if(this.id) {
