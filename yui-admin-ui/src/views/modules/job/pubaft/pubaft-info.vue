<template>
  <el-dialog title="预览" class="mod-pubaft-info details"
    ...
    <span v-if="$hasPerm('job:pubaft:audit') && details.statCd=='30070.120.100'" slot="footer" class="dialog-footer">
...
<script>
  import audit from './pubaft-audit'
  export default {
    data() {
      return {
        id: null,
    ...
    methods: {
      ...
      auditPass () {
          ...
          if(this.details.statCd==='30070.170.110') {
            this.$http.post('/job/pubaft/audit',{id:this.id,statCd:'30070.170.120'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
            ...
            this.$http.post('/job/pubaft/audit',{id:this.id,statCd:'30070.170.140'}).then(({data: res}) => {
              if (res && res.code === 0) {
                this.$message({
                  message: '审核成功',
                  type: 'success',
