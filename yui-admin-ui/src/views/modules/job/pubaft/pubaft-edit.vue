...
<script>
  ...
  export default {
    ...
    activated () {
        ...
        if(!this.id && this.$route.name === 'pubaft-edit')//编辑页面第一次进来获取传过来的id，防止点击tab页异常
          this.id = this.$route.params.id ;
        this.$refs['dataForm'].resetFields()
        if(this.id && this.$route.name === 'pubaft-edit')//编辑页获取用户信息
        {
           this.getInfo()  
        }
    },
    methods: {
      ...
      view () {
       this.$refs['dataForm'].validate((valid) => {
          if (valid) {
	        ...
	        this.details.fileDtoList = this.dataForm.jobPubaftFileRoList
	        this.$nextTick(() => {
      ...
      // 获取职称申报系统-政策文件表信息
      getInfo () {
        this.$http.get('/job/pubaft/webInfo?id=' + this.id).then(({data: res}) => {
          if (res.code !== 0) {
          ...
          this.dataForm.jobPubaftFileRoList = this.dataForm.fileDtoList
        }).catch(() => {})
      },
       // 获取编辑器内容 格式为html
      getEditConts (value) {
      ...
      uploadHandle (val) {
        ...
        this.$set(this.dataForm, 'jobPubaftFileRoList', val)
      },
      // tabs, 删除tab
      ...
      dataFormSubmit: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            //console.log(JSON.stringify(this.dataForm.jobPubaftFileRoList));
            let url = !this.id ? '/job/pubaft/add' : '/job/pubaft/upd'
            /*if(this.id) {
